<?php 
    ...
    class Destino{
        ...
        private $destNombre;
        private $regID;
        static $regNombre; //viene de la tabla regiones por eso va estatico porque el resultado no lo vamos a cambiar
        private $destPrecio;
        ...
        public function listarDestinos(){
            ...
            $sql = "SELECT destID, destNombre, d.regID, r.regNombre, destPrecio, destAsientos, destDisponibles, destActivo FROM destinos d, regiones r WHERE d.regID = r.regID"; //METODO RELATION TABLE
            //$sql = "SELECT * FROM destinos INNER JOIN regiones ON destinos.regID = regiones.regID"; //METODO JOIN
        ...
        public function verDestinoPorID(){
            $destID = $_GET['destID'];
            $link = Conexion::conectar();
            $sql = "SELECT destID, destNombre, d.regID, r.regNombre, destPrecio, destAsientos, destDisponibles, destActivo FROM destinos d, regiones r WHERE d.regID = r.regID AND destID = :destID"; //METODO RELATION TABLE CON PARAMETRO :DESTID
            //$sql = "SELECT destID, destNombre, regID, destPrecio, destAsientos, destDisponibles, destActivo FROM destinos WHERE destID = :destID";
            $stmt = $link->prepare($sql);
            $stmt->bindParam(':destID', $destID, PDO::PARAM_INT);
            $stmt->execute();
            $datos = $stmt->fetch(PDO::FETCH_ASSOC);

            // asigno valores a los setters para mostrarlo en ModificarDestino
            $this->setDestID($destID);
            $this->setDestNombre($datos['destNombre']);
            $this->setRegID($datos['regID']);
            self::setRegNombre($datos['regNombre']); //al ser una variable estatica la llamamos con self
        ...
        public function agregarDestino(){
            ...
            $link = Conexion::conectar();
            $sql = "INSERT INTO destinos (destNombre, regID, destPrecio, destAsientos, destDisponibles) VALUE (:destNombre, :regID, :destPrecio, :destAsientos, :destDisponibles)";
            $stmt = $link->prepare($sql);
            ...
            if ($stmt->execute()){
                ...
                $this->setDestID($link->lastInsertId()); //lastInsertId es para que levante el ultimo ID generado por el execute
        ...
        public function modificarDestino(){
            ...
            $link = Conexion::conectar();
            $sql= "UPDATE destinos SET destNombre = :destNombre,  regID = :regID, destPrecio = :destPrecio, destAsientos = :destAsientos, destDisponibles = :destDisponibles WHERE destID = :destID";
            $stmt = $link->prepare($sql);
            $stmt->bindParam(':destNombre', $destNombre, PDO::PARAM_STR);
            $stmt->bindParam(':regID', $regID, PDO::PARAM_INT);
            $stmt->bindParam(':destPrecio', $destPrecio, PDO::PARAM_INT);
